...
const CardNegocio = (props) => {
    ...
    useEffect(() => {
        if (userData.isLogged && props.favorites && props.favorites.length > 0) {
            ...
            if (favoritesArr.includes(userData.email)) heartRef.current.checked = true;
                ...
                <div className="card">
                    <figure style={{ position: "relative" }}>
                        <div className="favorito">
                            <input name="favorito" type="checkbox" ref={heartRef} defaultChecked={false}/>
                            <label className="bi bi-heart-fill favoriteHeart" onClick={handleFavorites}></label>
                        </div>
